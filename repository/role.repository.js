...
const roleRepository = () => {
  ...
  const getAllAndCount = async (args = {}) => {
    ...
    try {
      const data = await Role.find(query).skip(skip).limit(limit).sort(sort);
  ...
  const getAll = async (args) => {
    ...
    return Role.find(args).catch((err) => {
      errorService.throwError({
        err,
        operation,
        name,
  ...
  const create = async (user) => {
    ...
    try {
      ...
      let foundUser = await Role.findOne({ email });
      ...
      if (username) {
        let foundUser = await Role.findOne({ username });
        if (foundUser) {
          throw new ConflictError({
            message: strings.userExists,
            details: [strings.userExists],
  ...
  const update = async (args) => {
    ...
    try {
      ...
      const foundUser = await Role.findOne({ _id });
      ...
      const entity = await Role.findOneAndUpdate({ _id }, data, { new: true }).exec();
  ...
  const deleteById = (args = {}) => {
    ...
    try {
      ...
      return Role.findOneAndRemove({ _id }).exec();
    } catch (err) {
      errorService.throwError({
        err,
        operation,
